...
async function getData() {
  try {
    const dataSet = await axios.get(
      `https://opentdb.com/api.php?amount=30&category=${category}&difficulty=${difficulty}&type=multiple`
    );
    questionPool = dataSet.data.results;

    buildShuffleArray();
  } catch (error) {
    alert(`This is my api request error:  ${errer}`);
  }
}
...
const closeCat = (location) => {
  if (categoryClicked && difficultyClicked) {
    ...
    document.querySelector(
      ".high-score-num"
    ).innerHTML = sessionStorage.getItem("score");
  }
};
